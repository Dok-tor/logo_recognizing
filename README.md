...
1. Первым этапом мы вырежем логотип по внутренней части bounding box-а и приведём его к фиксированному размеру 224х224 пикселя.  
2. Далее мы сохраним полученные картинки в папки соответствующие названию бренда, чтобы получилось вот так:
```text
...
Такая модель после 11 эпох обучения на всём наборе данных (за исключением 4-х фотографий отложенных отдельно) уже показала способность разделять логотипы разных брендов.  
